...
1.  Apply for the [Student Developer Pack](https://education.github.com/pack) at GitHub using your UCLA email. You'll get GitHub Pro account for free (unlimited public and private repositories).
    ...
    I got GitHub Pro account.

    ![](images/Screenshot%202024-01-16%20at%2010.57.31.png){width="196"}
...
2.  Create a **private** repository `biostat-203b-2024-winter` and add `Hua-Zhou` and TA team (`Tomoki-Okuno` for Lec 1; `jonathanhori` and `jasenzhang1` for Lec 80) as your collaborators with write permission.
    ...
    ![](images/Screenshot%202024-01-16%20at%2011.17.41.png){width="325"}
...
3.  Top directories of the repository should be `hw1`, `hw2`, ... Maintain two branches `main` and `develop`. The `develop` branch will be your main playground, the place where you develop solution (code) to homework problems and write up report. The `main` branch will be your presentation area. Submit your homework files (Quarto file `qmd`, `html` file converted by Quarto, all code and extra data sets to reproduce results) in the `main` branch.
    ...
    \
    ![](images/Screenshot%202024-01-16%20at%2011.18.35-01.png)
...
**Answer:**These data files are distributed as ".csv.gz" files instead of ".csv" files because ".csv.gz" files are compressed files. Compressed files take up less storage space than uncompressed files. This is why these data files are distributed as ".csv.gz" files instead of ".csv" files. and use less bandwidth when transferred over the network. This is why these data files are distributed as ".csv.gz" files instead of ".csv" files.
...
".csv.gz" files reduce file size and saves storage space. That's why they are used instead of ".csv" files.

3.  Briefly describe what Bash commands `zcat`, `zless`, `zmore`, and `zgrep` do.

    (zcat)

    "zcat" can display the contents of compressed files. This means "zcat" displays the contents of a compressed file without actually decompressing it.

    (zless)

    "zless" can display the contents of compressed files in a scrollable manner. This means "zless" displays the contents of a compressed file without actually decompressing it.

    (zmore)

    "zmore" can display the contents of compressed files line by line. This means "zmore" displays the contents of a compressed file without actually decompressing it.

    (zgrep)

    "zgrep" can extract a specific string or pattern within a compressed files.
...
Compressed gz file doesn't take up much storage space, but it takes longer to run. Uncompressed file takes up more storage space, but it takes shorter to run. It's a trade off between storage and speed for big data files.
...
"wget" downloads files from some URL only if the file doesn't already exist locally or if the local file has to be updated. If the file exists and is up-to-date, "wget" will not download it again.
