...
Retrieve the hadm_id and stay_id of the patient.
...
labevents = open_dataset(sources = "./chartevents_pq",
  ...
  filter(subject_id %in% patient_id) |>
  filter(itemid %in% transfers$itemid) |>
  collect() |>
...
As the number of admissions per patient increases, the number of patients decreases. This looks right skewed. (Note that some patients have many admissions.) 
...
The number of admissions is highest at 00:00 and decreases until 06:00. However, it increases sharply again at 07:00, and then declines at 08:00. Thereafter, it gradually increases until 18:00. This might be related to the emergency admissions at midnight and in the morning.
...
There is a peak at 00 minutes, and then the peak happens every 15 minutes. These peaks are getting smaller as the minute increases. This might be related to the rounding issue of the admission time.
...
As the length of hospital stay increases, the number of events decreases.This looks right skewed. Many patients are discharged within days or weeks.
...
(Gender-wise Patient Counts)
The number of female patients is slightly higher than that of males.
...
patients_tble |>
  ...
  labs(title = "Gender-wise Patient Counts",
       x = "Gender",
       y = "Number of Patients") +
  theme_minimal()
```
(Density Plot)
The distribution of the anchor_age is almost the same for men and women. However, it seems like the number of female patients in their 20s and 30s is greater than that of male patients. Additionally, the number of male patients in their 40s and 70s is greater than that of female patients. Finally, the number of female patients in their 80s is greater than that of male patients.

```{r}
patients_tble |>
  ggplot(aes(x = anchor_age, color = gender)) +
  geom_density() +
  labs(title = "Density Plot by gender",
       x = "anchor_age",
       y = "Density")
```

(Box Plot)
They have similar distributions of anchor_age. however, it looks like the median of the anchor_age of the male patients is slightly higher than that of the female patients.

```{r}
patients_tble |>
  ggplot(aes(y = anchor_age, x = gender, color = gender)) +
  geom_boxplot() +
  labs(title = "Box Plot by gender",
       x = "Gender",
       y = "anchor_age")
```
...
Restrict to the last available measurement (by `storetime`) before the ICU stay. When I found the same "storetime", I chose the one with the latest "charttime". When I found the same "storetime" and "charttime", I chose the one with the maximum "valuenum".
...
labevents_sub2 <- labevents_sub |>
  ...
  # Measurement before the ICU stay
...
As for the graph, "race" are categorized into "ASIAN", "WHITE", "BLACK", "HISPANIC", and "OTHER". This is the box plot of the length of ICU stay by the category. 
...
gg_race_los <- race_los |>
  ...
  labs(title = "Length of ICU stay by race (without outlier)",
       x = "Race",
       y = "Length of ICU stay") +
...
insurance_los <- mimic_icu_cohort |>
  ggplot(aes(x = insurance, y = los)) +
  geom_boxplot(outlier.shape = NA) +
  labs(title = "Length of ICU stay by insurance (without outlier)",
       x = "Insurance",
       y = "Length of ICU stay") +
...
marital_status_los <- mimic_icu_cohort |>
  ...
  ggplot(aes(x = marital_status, y = los)) +
  geom_boxplot(outlier.shape = NA) +
  labs(title = "Length of ICU stay by marital status (without outlier)",
       x = "Marital status",
       y = "Length of ICU stay") +
...
gender_los <- mimic_icu_cohort |>
  ggplot(aes(x = gender, y = los)) +
  geom_boxplot(outlier.shape = NA) +
  labs(title = "Length of ICU stay by gender (without outlier)",
       x = "Gender",
       y = "Length of ICU stay") +
...
```{r, message=FALSE}
vital_abb <- read_csv("~/mimic/icu/d_items.csv.gz") |>
  filter(abbreviation == "NBPd" | abbreviation == "NBPs" |
           abbreviation == "HR" | abbreviation == "Temperature F" |
           abbreviation == "RR") |>
...
vital_los <- mimic_icu_cohort |>
  select(los, heart_rate, non_invasive_blood_pressure_systolic,
         ...
         respiratory_rate) |>
  rename(setNames(gsub(" ", "_", tolower(vital_abb$label)),
                  vital_abb$abbreviation)) |>
...
Considering the outliers, the scatter plot is as follows. (I set the limit of the x-axis to 99.9% quantile.)

```{r, warning=FALSE}
q <- 0.999

vital_los_trim <- mimic_icu_cohort |>
  select(los, heart_rate, non_invasive_blood_pressure_systolic,
         non_invasive_blood_pressure_diastolic, temperature_fahrenheit,
         respiratory_rate) |>
  filter(heart_rate < quantile(heart_rate, q, na.rm = TRUE)) |>
  filter(non_invasive_blood_pressure_systolic <
           quantile(non_invasive_blood_pressure_systolic, q,
                    na.rm = TRUE)) |>
  filter(non_invasive_blood_pressure_diastolic <
           quantile(non_invasive_blood_pressure_diastolic, q,
                    na.rm = TRUE)) |>
  filter(temperature_fahrenheit < quantile(temperature_fahrenheit, q,
                                           na.rm = TRUE)) |>
  filter(respiratory_rate < quantile(respiratory_rate, q, na.rm = TRUE)) |>
  rename(setNames(gsub(" ", "_", tolower(vital_abb$label)),
                  vital_abb$abbreviation)) |>
  pivot_longer(cols = - los, names_to = "vital", values_to = "value") |>
  ggplot(aes(x = value, y = los)) +
