...
author: "Hiroyasu Ando, 605948443"
format:
  html:
    theme: cosmo
...
Put "labevents.parquet" in the current working directory `hw3` as "labevents_pq".

```{bash}
#| eval: false
cp -r ../hw2/labevents.parquet ./labevents_pq
```
...
Retrieve the patient's top 3 diagnoses.

```{r, message=FALSE}
patient_diagnoses <- read_csv("~/mimic/hosp/diagnoses_icd.csv.gz") |>
  left_join(read_csv("~/mimic/hosp/d_icd_diagnoses.csv.gz"), by = c("icd_code", "icd_version")) |>
  filter(subject_id %in% patient_id) |>
...
```{bash}
#| eval: false
cp -r ../hw2/chartevents.parquet ./chartevents_pq
```
...
As the length of hospital stay increases, the number of patients decreases. The most common length of hospital stay is a few days.
...
Men and women have the similar patterns. The number of patients in their early 20s, both male and female, is high. Also, the number of female patients is higher than the number of male patients.
...
labevents_q5 = open_dataset(sources = "./labevents_pq", format = "parquet") |>
...
```{r, message=FALSE, warning = FALSE}
#labevents_q5_2 = labevents_q5 |>
  #select(subject_id, itemid, valuenum, storetime, stay_id, intime, charttime) |>
  #filter(storetime < intime) |>
  #arrange(subject_id, stay_id, itemid, storetime, charttime, valuenum) |>
...
```{r, message=FALSE, warning = FALSE}
chartevents_q6 = open_dataset(sources = "./chartevents_pq",
                              format = "parquet") |>
  select(subject_id, itemid, valuenum, storetime, charttime) |>
...
```{r}
chartevents_q6_2 = chartevents_q6 |>
  select(subject_id, itemid, valuenum, storetime, stay_id, intime, charttime, outtime) |>
...
```{r}
chartevents_q6_3 = chartevents_q6_2 |>
  pivot_wider(names_from = itemid, values_from = valuenum) |>
  arrange(subject_id, stay_id) |>
  ungroup() |>
  rename(heart_rate = "220045", non_invasive_blood_pressure_systolic = "220179",
         non_invasive_blood_pressure_diastolic= "220180",
         temperature_fahrenheit = "223761", respiratory_rate = "220210") |>
  print(width = Inf)
```
...
mimic_icu_cohort = icustays_tble |>
  ...
  left_join(labevents_q5_3, by = c("subject_id", "stay_id")) |>
  left_join(chartevents_q6_3, by = c("subject_id", "stay_id")) |>
