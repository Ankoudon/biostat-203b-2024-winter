...
Random forest : hematocrit 

XGBoost : temperature fahrenheit

```{r}
# Logistic regression
logit_final_wf |> fit(mimiciv_icu_cohort_other) |>
  extract_fit_parsnip() %>%
  tidy() |>
  print(n = Inf)

# Random forest
rf_final_wf |> fit(mimiciv_icu_cohort_other) |>
  extract_fit_parsnip() |>
  vip::vi() 

# XGBoost
gb_final_wf |> 
  fit(mimiciv_icu_cohort_other) |>
  extract_fit_parsnip() |> 
  vip::vi() 
```

The stacking model has better accuracy and AUC than the three individual models. Based on the coefficients of the stacking model, random forest and XGBoost help improve predictions, compared to logistic regression. Considering this discussion, the most important feature in predicting long-term ICU stays may be the decision tree, which implies a non-liner and complex relationship.
