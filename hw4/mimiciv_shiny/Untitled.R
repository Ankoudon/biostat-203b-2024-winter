...
ui <- fluidPage(
  ...
  tabsetPanel(
    ...
    tabPanel("Tab 1",
             sidebarPanel(
             selectInput("v_1", "Summary", 
                         c("demographics" = "demographics",
                           "lab measurements" = "lab",
                           "vitals" = "vitals"
                           ))),
             #radioButtons("type_1", "Summary Type:",
                          #choices = c("Numeric", "Graphical")),
...
server <- function(input, output, session) {
    ...
    if (type == "demographics") {
      ...
      mean_table <- mimic_icu_cohort_pre %>%
        select(creatinine,
               ...
               glucose) %>%
        na.omit() %>%
        summarise_all(~ round(mean(., na.rm = TRUE), digits = 1)) |> gt() |>
        tab_header(
      ...
      mean_table <- mimic_icu_cohort_pre %>%
        select(heart_rate, non_invasive_blood_pressure_systolic,
               non_invasive_blood_pressure_diastolic, temperature_fahrenheit,
               respiratory_rate) %>%
        rename("NBPd" = non_invasive_blood_pressure_diastolic,
               "NBPs" = non_invasive_blood_pressure_systolic,
               ...
               "RR" = respiratory_rate) %>%
        na.omit() %>%
        summarise_all(~ round(median(., na.rm = TRUE), digits = 1)) |> gt() |>
        tab_header(
  ...
  output$view <- render_gt({
    
    #if (input$type_1 == "Numeric") {
